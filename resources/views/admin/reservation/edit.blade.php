...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin/select_seat_edit', $reservation->id) }}">
                    @csrf
                        ...
                        <div class="col-md-6">
                            <select id="user_id" class="form-select @error('user_id') is-invalid @enderror" name="user_id" aria-label="Default select example" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ ($user->id == $reservation->user_id)? 'selected': '' }}>{{ $user->name }} {{ $user->surname }}</option>
                                @endforeach
                            </select>
